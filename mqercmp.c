...
/*   2014年10月07日 08時03分07秒 -                                            */
/*  3. Body, Japanese                                                         */
/*   AMQnnnn: DBCS characters will end with "。"                              */
/*  4. Body, English                                                          */
...
/*  or iconv for prepare proper charset of input file.                        */ 
/*                                                                            */
...
// fp = fopen(stdin, "r");
 while(fgets(ibuf, IBUF_LEN-1, stdin)){
    ...
    if(isdigit(ibuf[0])){                        // should be Date and Time
...
{
 if(strstr(p, TERM_EN)) return 0;  // end of English sentence
 if(strstr(p, TERM_JP)) return 0;  // end of Japanse sentence
